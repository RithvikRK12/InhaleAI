"""
AI-Assisted Alternate Nostril Breathing (Nadi Shodhana) Guide - Flask Version
Adapted for web streaming with Flask
"""
...
class NadiShodhanaGuide:
    def __init__(self):
        ...
        self.target_breath_duration = 5.0  # Target 4-6 seconds (healthier range)
        ...
        self.circle_max_radius = 120
        self.circle_min_radius = 30
        self.breathing_animation_phase = 0
        self.breathing_speed = 0.5  # Slower breathing animation
    ...
    def detect_nostril_closure(self, hand_landmarks, face_landmarks):
        """Enhanced nostril detection with better accuracy and stability"""
        if not hand_landmarks or not face_landmarks:
            return None
        
        # Get multiple hand landmarks for better detection
        ...
        # Calculate distances to nose area
        distances = {
            'thumb': math.sqrt((thumb_tip.x - nose_tip.x)**2 + (thumb_tip.y - nose_tip.y)**2),
            'index': math.sqrt((index_tip.x - nose_tip.x)**2 + (index_tip.y - nose_tip.y)**2),
            'middle': math.sqrt((middle_tip.x - nose_tip.x)**2 + (middle_tip.y - nose_tip.y)**2),
            'ring': math.sqrt((ring_tip.x - nose_tip.x)**2 + (ring_tip.y - nose_tip.y)**2)
        }
        
        # Find closest finger to nose
        closest_finger = min(distances, key=distances.get)
        min_distance = distances[closest_finger]
        
        # More sensitive detection threshold
        if min_distance < 0.08:  # Reduced from 0.1 for better sensitivity
            finger_landmark = {
                'thumb': thumb_tip,
                'index': index_tip,
                'middle': middle_tip,
            ...
            # Determine which nostril based on finger position relative to nose
            nose_center_x = nose_tip.x
            finger_x = finger_landmark.x
            
            # Add vertical position check for better accuracy
            nose_center_y = nose_tip.y
            finger_y = finger_landmark.y
            
            # Check if finger is close enough vertically
            vertical_distance = abs(finger_y - nose_center_y)
            if vertical_distance < 0.15:  # Within reasonable vertical range
                # Add stability check - require consistent detection
                if not hasattr(self, 'nostril_detection_history'):
                    self.nostril_detection_history = []
                
                # Determine nostril based on finger position
                if finger_x < nose_center_x - 0.02:  # Left side of nose
                    detected_nostril = "right"  # Finger on left side closes right nostril
                elif finger_x > nose_center_x + 0.02:  # Right side of nose
                    detected_nostril = "left"   # Finger on right side closes left nostril
                else:
                    return None
                
                # Add to history for stability
                self.nostril_detection_history.append(detected_nostril)
                if len(self.nostril_detection_history) > 5:
                    self.nostril_detection_history.pop(0)
                
                # Require consistent detection (at least 3 out of 5 recent detections)
                if len(self.nostril_detection_history) >= 3:
                    most_common = max(set(self.nostril_detection_history), key=self.nostril_detection_history.count)
                    if self.nostril_detection_history.count(most_common) >= 3:
                        return most_common
        
        return None
    
    def detect_breathing_pattern_microphone(self):
        """Detect breathing pattern using microphone for enhanced accuracy"""
        ...
        try:
            if not self.audio_queue.empty():
                ...
                # Enhanced breathing detection with hysteresis
                noise_threshold = 1000  # Adjust based on environment
                inhale_threshold = noise_threshold * 1.3
                exhale_threshold = noise_threshold * 0.7
                
                current_time = time.time()
                
                # Add minimum duration check to prevent false triggers
                min_breath_duration = 1.0  # Minimum 1 second for valid breath
                
                # Detect breathing phases based on audio amplitude
                if amplitude > inhale_threshold:
                    if self.breathing_phase != "inhale":
                        # Only start inhale if we've been in exhale for minimum duration
                        if hasattr(self, 'breath_start_time') and (current_time - self.breath_start_time) >= min_breath_duration:
                            self.breathing_phase = "inhale"
                            self.breath_start_time = current_time
                            print(f"Microphone detected: INHALE (amplitude: {amplitude:.1f})")
                        elif not hasattr(self, 'breath_start_time'):
                            self.breathing_phase = "inhale"
                            self.breath_start_time = current_time
                            print(f"Microphone detected: INHALE (amplitude: {amplitude:.1f})")
                elif amplitude < exhale_threshold:
                    if self.breathing_phase == "inhale":
                        breath_duration = current_time - self.breath_start_time
                        # Only record if duration is reasonable
                        if breath_duration >= min_breath_duration:
                            self.breath_durations.append(breath_duration)
                            print(f"Microphone detected: EXHALE (duration: {breath_duration:.1f}s)")
                        self.breathing_phase = "exhale"
                        self.breath_start_time = current_time
    ...
    def check_breathing_rhythm(self):
        """Check breathing rhythm and provide feedback with healthier 4-6 second target"""
        ...
        feedback = "good"
        if avg_duration < 3.0:  # Too fast - unhealthy
            feedback = "too_fast"
            self.trigger_alert("breathing", "Slow down your breathing - aim for 4-6 seconds")
        elif avg_duration > 7.0:  # Too slow - might cause dizziness
            feedback = "too_slow"
            self.trigger_alert("breathing", "Breathe a bit faster - aim for 4-6 seconds")
        elif avg_duration < 4.0:  # Slightly fast
            feedback = "slightly_fast"
            self.trigger_alert("breathing", "Good rhythm, try to slow down slightly")
        elif avg_duration > 6.0:  # Slightly slow
            feedback = "slightly_slow"
            self.trigger_alert("breathing", "Good rhythm, try to speed up slightly")
        
        return feedback == "good", feedback
    
    def draw_breathing_guide(self, img):
        """Draw visual breathing guide circle with smooth, healthy pace"""
        h, w = img.shape[:2]
        center = (w - 100, 100)
        
        # Use session start time for consistent breathing cycle
        if not hasattr(self, 'session_start_time') or self.session_start_time is None:
            self.session_start_time = time.time()
        
        # Smooth breathing animation with healthy pace
        current_time = time.time()
        breathing_cycle_duration = 8.0  # 8 seconds total (4 inhale + 4 exhale)
        cycle_progress = ((current_time - self.session_start_time) % breathing_cycle_duration) / breathing_cycle_duration
        
        if cycle_progress < 0.5:  # First half = inhale
            self.breathing_phase = "inhale"
            # Smooth inhale animation
            inhale_progress = cycle_progress * 2  # 0 to 1
            self.circle_radius = int(self.circle_min_radius + (self.circle_max_radius - self.circle_min_radius) * inhale_progress)
            color = (0, 255, 0)  # Green for inhale
        else:  # Second half = exhale
            self.breathing_phase = "exhale"
            # Smooth exhale animation
            exhale_progress = (cycle_progress - 0.5) * 2  # 0 to 1
            self.circle_radius = int(self.circle_max_radius - (self.circle_max_radius - self.circle_min_radius) * exhale_progress)
            color = (0, 0, 255)  # Red for exhale
        
        # Draw breathing circle with gradient effect
        for i in range(3):
            alpha = 1.0 - (i * 0.3)
            radius = int(self.circle_radius * (1 - i * 0.1))
        ...
        cv2.putText(img, "Target: 4-6 sec", (w - 180, h - 20),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Draw breathing rhythm indicator
        rhythm_text = f"Rhythm: {self.circle_radius - self.circle_min_radius}%"
        cv2.putText(img, rhythm_text, (w - 180, h - 40),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        
        # Draw cycle progress
        progress_text = f"Cycle: {cycle_progress*100:.1f}%"
        cv2.putText(img, progress_text, (w - 180, h - 60),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        
        # Store cycle progress for UI sync
        self.cycle_progress = cycle_progress
        self.circle_scale = (self.circle_radius - self.circle_min_radius) / (self.circle_max_radius - self.circle_min_radius)
    ...
    def start_camera(self):
        ...
        if not self.camera_active:
            ...
            print("Camera started for web streaming")
    ...
    def get_frame(self):
        """Get a single frame for web streaming"""
        ...
        # Enhanced breathing detection using microphone
        self.detect_breathing_pattern_microphone()
    ...
    def start_breathing_sequence(self):
        """Start the guided breathing sequence"""
        self.nostril_index = 0
